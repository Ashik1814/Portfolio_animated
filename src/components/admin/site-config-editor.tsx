...
interface SiteConfigEditorProps {
  ...
  defaultCollapsed?: string; // group id to expand initially when SEO is selected
}

interface FieldGroup {
  id: string;
  title: string;
  fields: { key: keyof SiteConfig; label: string; type: "input" | "textarea" }[];
}
...
const FIELD_GROUPS: FieldGroup[] = [
  {
    ...
    fields: [
      ...
      { key: "cvUrl", label: "CV / Resume Download URL", type: "input" },
...
export function SiteConfigEditor({ config, onSave, defaultCollapsed }: SiteConfigEditorProps) {
  ...
  const [collapsedGroups, setCollapsedGroups] = useState<Record<string, boolean>>(() => {
    if (defaultCollapsed) {
      // When SEO is selected, collapse all groups EXCEPT the seo group
